...
class TestZeekConfigHandler(unittest.TestCase):
    ...
    def test_default_initialization_static_analysis(self):
        ...
        # Assert
        self.assertEqual(
            handler.base_config_location, "/usr/local/zeek/share/zeek/site/local.zeek"
        )
    ...
    def test_default_initialization_network_analysis(self):
        self.mock_config["pipeline"]["zeek"]["sensors"]["ZEEK_TEST_CONTAINER"][
            "static_analysis"
        ] = False
        handler = ZeekConfigurationHandler(self.mock_config)
        # Assert
    ...
    def test_configure_default_mode(self, mock_exists, mock_open):
        handler = ZeekConfigurationHandler(self.mock_config)
        # Arrange
        pipline_in_topic_prefix = self.mock_config["environment"][
            "kafka_topics_prefix"
    ...
    def test_create_plugin_configuration(self):
        ...
        mock_config = {
            ...
            "pipeline": {
                "zeek": {"sensors": {"test_container": {"protocols": ["http", "dns"]}}}
            },
        }
        ...
        args, kwargs = handle.writelines.call_args_list[
            0
        ]  # args is a tuple of positional args
        written_lines = "".join(
            args[0]
        )  # the iterable passed to writelines concatenated to a single string
    ...
    def test_template_and_copy_node_config(self, mock_shutil_copy, mock_open_file):
        ...
        mock_shutil_copy.assert_called_once_with(
            "/opt/src/zeek/base_node.cfg", "/usr/local/zeek/etc/node.cfg"
        )
