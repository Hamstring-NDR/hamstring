...
class TestDGADetector(unittest.TestCase):
    ...
    def test_detect(self):
        ...
        with patch(
            "src.detector.plugins.dga_detector.DGADetector.predict",
    ...
    def test_entropy_calculation(self):
        ...
        self.assertAlmostEqual(
            detector._get_features(uniform)[0][-3], uniform_entropy, delta=0.01
        )

        # Test with repetitive pattern (low entropy)
        repetitive = "aaaa"
        self.assertAlmostEqual(
            detector._get_features(repetitive)[0][-3], 0.0, delta=0.01
        )
